...
class AutoLoginAction(Action):
    ...
    def input_username_password(self, connection, waittime):
        """
        About login, need try one more time, some times the serial output
        makes chaos, the pexpect session can't caught the expected content.
        """
        params = self.parameters.get('auto_login')
        self.logger.debug("Sending enter wait for login: again")
        connection.sendline('\n\n', delay=self.character_delay)
        index = self.wait(connection, waittime)
        if index:
            self.logger.debug("Matched prompt #%s: %s", index, connection.prompt_str[index])
            if connection.prompt_str[index] in [LOGIN_INCORRECT_MSG, LOGIN_TIMED_OUT_MSG]:
                return index 
    ...
    def run(self, connection, max_end_time):
        ...
        if not params:
            ...
            for i in range(1, count+1):
                waittime = int((max_end_time - time.time())/count + time.time())
                index = self.input_username_password(connection, waittime)
                if connection.prompt_str[index] not in [LOGIN_INCORRECT_MSG, LOGIN_TIMED_OUT_MSG]:
                    break
