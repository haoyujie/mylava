...
class TestShellAction(TestAction):
    ...
    def add_reboot_action(self):
        '''add minimal_boot/uboot_action'''
        index = 0
        boot_action = None
        parent_action = None
        boot_action_names  = ['minimal-boot','uboot-action']
        boot_action_names += ['uboot-from-media','bootloader-overlay','uboot-retry','reset-device','auto-login-action']
        for jobact in self.job.pipeline.actions:
            if jobact.name in boot_action_names:
                boot_action = jobact
            if self in jobact.pipeline.actions:
                parent_action = jobact
        index = self.job.pipeline.actions.index(parent_action) 
        for act in boot_action.pipeline.actions:
            if act.name in boot_action_names:
                index += 1
                self.job.pipeline.actions.insert(index, act)
        self.logger.debug("TestShellAction : str(self.job.pipeline.actions) = %s ", str(self.job.pipeline.actions) )
            ...
            with connection.test_connection() as test_connection:
                ...
                while self._keep_running(test_connection, test_connection.timeout, connection.check_char):
                    ...
                    if feedbacks and time.time() - last_check > test_connection.timeout:
                        ...
                        last_check = time.time()
                    spending_time = time.time() - start_check
                    if spending_time > self.timeout.duration - 60:
                        self.logger.debug("TestShellAction: spending time = %s timeout = %s", spending_time, self.timeout.duration )
                        self.add_reboot_action()
                        break
