...
from lava_extra.virt_target_utils import (
    Virtlm,
    Virtils,
)

class VirtDeploy(Deployment):
    """
    Strategy class for a virt ramdisk based Deployment.
    ...
    def accepts(cls, device, parameters):
        ...
        if parameters['to'] != 'virt':
            return False, '"to" parameter is not "virt"'
        if 'virt' in device['actions']['deploy']['methods']:
            return True, 'accepted'
        return False, '"virt" was not in the device configuration deploy methods"'

class VirtDeployAction(Action):
    """
    Base class for all virt deploy actions which copy images (kernel,rootfs,dtb)
...
class VirtDeployMasterAction(VirtDeployAction):
    """
    Runs virttool copyfile master kernel image to deploy ramdisk image and boot
    """
    name = "deploy-master-image"
    description = "deploy ramdisk image to lab server"
    summary = "copy ramdisk image via virttool"
    ...
    def deploy_image(self):
        ...
        """
        self.logger.debug("Copy files to virtual device path")
...
#            kernel_tmp = re.findall('ccm1(.*)', kernel_url)[0]
...
#            rootfs_tmp = re.findall('^.*buildarea1/lava', kernel_tmp)[0]+re.findall('8080(.*)', rootfs_url)[0]
        ...
        if not self.virttool.copy_files(self.device_barcode, kernel=kernel_tmp, rootfs=rootfs_tmp, dtb = dtb_tmp):
    ...
    def run(self, connection, max_end_time):  # pylint: disable=too-many-locals
        ...
        if self.deploy_image() is not True:
            ...
            raise JobError('kernel or rootfs file not exist')
...
class VirtDeploySlaveAction(VirtDeployAction):
    """
    Runs virttool copyfile master kernel image to deploy ramdisk image and boot
    """
    ...
    def target_extract(self, connection, dest):
        ...
        storage_suffix = self.get_namespace_data(action='virt-deploy', label='storage', key='suffix')
