...
from lava_extra.simics_target_utils import (
    Simlm,
    Simtils,
)

class SimicsDeploy(Deployment):
    """
    Strategy class for a Simics ramdisk based Deployment.
    Downloads the relevant parts, copies to the virt location.
    ...
    def accepts(cls, device, parameters):
        ...
        if parameters['to'] != 'simics':
            return False, '"to" parameter is not "simics"'
        if 'simics' in device['actions']['deploy']['methods']:
            return True, 'accepted'
        return False, '"simics" was not in the device configuration deploy methods"'

class SimicsDeployAction(Action):
    """
    Base class for all simics deploy actions which copy images (kernel,rootfs,dtb)
...
#        if not self.virttool.reserve_target(self.device_barcode):
        #if not self.virttool.reserve_target(self.device_name):
...
class SimicsDeployMasterAction(SimicsDeployAction):
    """
    Runs simics virttool copyfile master kernel image to deploy ramdisk image and boot
    """
    name = "deploy-master-image"
    description = "deploy ramdisk image to lab server"
    summary = "copy ramdisk image via simics virttool"
    ...
    def deploy_image(self):
        ...
        """
        self.logger.debug("Copy files to simics virtual device path")
        ...
        if 'kernel' in self.parameters:
            self.logger.debug("enter none master branch")
            kernel_url = self.parameters.get('kernel')
            kernel_tmp = kernel_url
            rootfs_url = self.parameters.get('rootfs').get('url')
            rootfs_tmp = rootfs_url
        elif 'master' in self.parameters:
            self.logger.debug("enter master branch")
            kernel_tmp = self.parameters.get('master')

        if 'tags' in self.job.parameters:
            job_tag = self.job.parameters.get('tags')[0]
        else:
            job_tag = None

        self.virttool.power_off_target(self.device_barcode)
        if not self.virttool.copy_files(self.device_barcode, kernel=kernel_tmp, rootfs=rootfs_tmp, dtb = dtb_tmp, tag=job_tag):
           # if self.virttool.error_str:
           #     self.logger.error(self.virttool.error_str)
                return False
        return True
...
class SimicsDeploySlaveAction(SimicsDeployAction):
    """
    Runs simics virttool copyfile master kernel image to deploy ramdisk image and boot
    """
...
#            self.update_pxeconfig()
    ...
    def target_extract(self, connection, dest):
        ...
        storage_suffix = self.get_namespace_data(action='simics-deploy', label='storage', key='suffix')
...
class SimicsAction(DeployAction):  # pylint:disable=too-many-instance-attributes

    name = "simics-deploy"
    description = "download images and deploy using simics virttool"
    summary = "simics deployment"
    ...
    def populate(self, parameters):
        ...
        self.internal_pipeline.add_action(SimicsDeployMasterAction())
        ...
        if 'rootfs_path' in parameters:
            ...
            self.internal_pipeline.add_action(SimicsDeploySlaveAction())
