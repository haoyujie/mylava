...
from lava_extra.qemu_target_utils import (
    Qemulm,
    Qemuils,
)

class QemudevDeploy(Deployment):
    """
    Strategy class for a Qemudev ramdisk based Deployment.
    ...
    def accepts(cls, device, parameters):
        ...
        if parameters['to'] != 'qemudev':
            return False, '"to" parameter is not "qemudev"'
        if 'qemudev' in device['actions']['deploy']['methods']:
            return True, 'accepted'
        return False, '"qemudev" was not in the device configuration deploy methods"'

class QemudevDeployAction(Action):
    """
    Base class for all qemudev deploy actions which copy images (kernel,rootfs,dtb)
...
class QemudevDeployMasterAction(QemudevDeployAction):
    """
    Runs qemudev virttool copyfile master kernel image to deploy ramdisk image and boot
    """
    name = "deploy-master-image"
    description = "deploy ramdisk image to lab server"
    summary = "copy ramdisk image via qemudev virttool"
    ...
    def deploy_image(self):
        ...
        """
        self.logger.debug("Copy files to qemudev virtual device path")
        ...
        self.virttool.power_off_target(self.device_name)
        #self.logger.debug(str(self.job.parameters.get('context').get('cmd')))
        if not self.virttool.copy_files(self.device_name, kernel=kernel_tmp, rootfs=rootfs_tmp, dtb = dtb_tmp, tag=job_tag):
...
class QemudevDeploySlaveAction(QemudevDeployAction):
    """
    Runs qemudev virttool copyfile master kernel image to deploy ramdisk image and boot
    """
...
class QemudevAction(DeployAction):  # pylint:disable=too-many-instance-attributes

    name = "qemudev-deploy"
    description = "download images and deploy using qemudev virttool"
    summary = "qemudev deployment"
