...
class GitRepoAction(RepoAction):  # pylint: disable=too-many-public-methods
    ...
    def run(self, connection, max_end_time):
        ...
        def unpack_actions(actions): 
            t_jobact = None
            for jobact in actions:
                if jobact.name == 'test-definition':
                    t_jobact = jobact
                elif jobact.pipeline is not None:
                    t_jobact = unpack_actions(jobact.pipeline.actions)
                if t_jobact is not None:
                    break
            return t_jobact

        latest_act = None
        test_definition_action = unpack_actions(self.job.pipeline.actions)
        for act in test_definition_action.pipeline.actions:
            if act.name == 'git-repo-action':
                if len(act.results) != 0:
                    if self.parameters['repository'] == act.results.get('repository'):
                        latest_act = act
                        break

        if latest_act is not None:
            ''' runner_path '/var/lib/lava/dispatcher/tmp/245/lava-overlay-r3gpkkz6/lava-245/0/tests/0_smoke-tests' 
                got latest_act runner_path
                link latest_act git case directory as current's '''
            runner_path.split('/')
            case_dir_head = self.parameters['path'].split('/')[0]
            dir_list = runner_path.split('/')[:-3]
            dir_list.append(str(latest_act.stage))
            dir_list.append('tests')
            dir_list.append('%s_%s' % (int(int(latest_act.level.split('.')[-1])/4),latest_act.parameters['name']))
            src_dir = '/'.join(dir_list)
            link_cmd  = "mkdir -p %s;" % (runner_path)
            link_cmd += "ln -rs %s %s;" % (src_dir + '/.git', runner_path + '/.git')
            link_cmd += "ln -rs %s %s" % (src_dir + '/' + case_dir_head, runner_path + '/' + case_dir_head)
            os.system(link_cmd)
            commit_id = latest_act.results.get('commit')
        else:
